...
var _default = function _default() {
  _svg.default.CircularArc = _svg.default.invent({
    // fill = line or full, mode = half or full
    create: function create(config) {
      // To Do...make sure min < max, initial is <= max and >= min
      this.max = config.max;
      this.min = config.min; //this.setting = config.setting;
      ...
      this.startAngle = 0;
      if (this.mode === 'half') this.startAngle = -90.0;
    ...
    extend: {
      ...
      angle: function angle() {
        var max = this.max;
        var min = this.min;
        var normal = (this.attr('setting') - min) / (max - min); //var denomral = (normal * (max - min) + min);
    ...
    construct: {
      CircularArc: function CircularArc(config) {
        ...
        return this.put(new _svg.default.CircularArc(config)).plot();
      }
    }
  });
};
